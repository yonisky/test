...
def move_file(id, a, b, overwrite):
    ...
    if overwrite==True:
        try:
            ...
            pyautogui.hotkey('alt', 'r', interval=0.5)
             #win32gui.PostMessage(win2._hWnd, win32con.WM_CLOSE, 0, 0)
             #window = uiautomation.WindowControl(searchDepth=2, Name='파일 바꾸기 또는 건너뛰기')
             #window.ButtonControl(Name="대상 폴더의 파일 덮어쓰기(R)").Click()
...
def empty_recycle(path):
    ...
    pyautogui.hotkey('shift', 'f10')
    time.sleep(3)
    uiautomation.MenuItemControl(searchDepth=2, Name='휴지통 비우기(B)').Click()
    #pyautogui.typewrite('by', interval=0.5) , 윈도우 11 새 탭에서열기(B) 와 겹침
    time.sleep(0.5)
    window.ButtonControl(Name="예(Y)").Click()
    time.sleep(3)
    win32gui.PostMessage(win._hWnd, win32con.WM_CLOSE, 0, 0)
...
def weblink(web_version, path, file):
    ...
    #options = webdriver.ChromeOptions()                                                   #크롬드라이버 옵션 부가
    #options.add_experimental_option('excludeSwitches', ['enable-logging'])
    #options.add_experimental_option('excludeSwitches', ['disable-software-rasterizer'])
    #driver = webdriver.Chrome(options=options)

    print('Google Chrome version is '+driver.capabilities['browserVersion']+'<BR>')
    driver.get(pyperclip.paste())
    if web_version=='3':
        #셀리니움driver.find_element_by_xpath('//*[@id="BodyWrap"]/div[3]/div[2]/div[2]/table/tbody/tr[1]/td/a').click() #81번 서버 전용 xpath
        driver.find_element(By.XPATH,'//*[@id="BodyWrap"]/div[3]/div[2]/div[2]/table/tbody/tr[1]/td/a').click() #81번 서버 전용 xpath

    elif web_version=='2':
        #셀리니움driver.find_element_by_xpath('//*[@id="BodyWrap"]/table/tbody/tr[2]/td/a').click() #183번 서버 전용 xpath
        driver.find_element(By.XPATH,'//*[@id="BodyWrap"]/table/tbody/tr[2]/td/a').click() #183번 서버 전용 xpath
...
def pre_export(web_version, addr, id, pw, path, file):
    ...
    if web_version == '3':
        # 크롬 드라이버로 승인 진행 230524
        driver = webdriver.Chrome(ChromeDriverManager().install()) #log_level=logging.ERROR
        driver.implicitly_wait(10)
        main_window = driver.window_handles[0]
        driver.get("http://"+addr+"/websvc/index/login.jsp")
        driver.implicitly_wait(10)
        #셀리니움driver.find_element_by_class_name('btn.btn-success').click()
        #셀리니움driver.find_element_by_id('id').send_keys(id)
        #셀리니움driver.find_element_by_id('pass').send_keys(pw)
        #셀리니움driver.find_element_by_class_name('btn.btn-success.btn-block.btn-next.d-ib.m-t.login-btn').click()
        #셀리니움driver.get("http://"+addr+"/filtersvc2/approval_doc_export_list.jsp")
        #셀리니움driver.find_element_by_xpath('//*[@id="Approvalsearch"]/div[3]/div/div/div[2]/div[1]/table/tbody/tr[1]/td[3]/a').click()
        #셀리니움driver.find_element_by_id('approve').click()
        #셀리니움driver.implicitly_wait(10)
        #셀리니움driver.find_element_by_name('reason').send_keys("rpa_"+export_time.strftime("%Y-%m-%d_%H%M%S"))
        #셀리니움driver.find_element_by_id('btnSelect').click()
        ...
        driver.find_element(By.CLASS_NAME,'btn.btn-success.btn-block.btn-next.d-ib.m-t.login-btn').click()
        driver.get("http://"+addr+"/filtersvc2/approval_doc_export_list.jsp")
        driver.find_element(By.XPATH,'//*[@id="Approvalsearch"]/div[3]/div/div/div[2]/div[1]/table/tbody/tr[1]/td[3]/a').click()
        driver.find_element(By.ID,'approve').click()
        driver.implicitly_wait(10)
        driver.find_element(By.NAME,'reason').send_keys("rpa_"+export_time.strftime("%Y-%m-%d_%H%M%S"))
        ...
        driver.implicitly_wait(10)
        #셀리니움driver.switch_to.frame(driver.find_element_by_name('bottom'))
        #셀리니움driver.find_element_by_name('id').send_keys(id)
        #셀리니움driver.find_element_by_name('pass').send_keys(pw)
        #셀리니움driver.find_element_by_xpath('//*[@id="Info_Tab_Contents"]/table/tbody/tr/td[9]/input').click()
        #셀리니움driver.find_element_by_id('disklock').click()
        #셀리니움driver.find_element_by_xpath('//*[@id="Doc_Menu"]/table/tbody/tr/td[2]/a/img').click()
        #셀리니움driver.find_element_by_xpath('//*[@id="BodyWrap_2"]/div/div[3]/form/table/tbody/tr/td[3]/a/span').click()
        #셀리니움driver.find_element_by_name('Reason').send_keys("rpa_"+export_time.strftime("%Y-%m-%d_%H%M%S"))
        #셀리니움driver.find_element_by_xpath('//*[@id="BodyWrap_2"]/div/div[3]/div[1]/img[2]').click()

        driver.switch_to.frame(driver.find_element(By.NAME,'bottom'))
        driver.find_element(By.NAME,'id').send_keys(id)
        driver.find_element(By.NAME,'pass').send_keys(pw)
        driver.find_element(By.XPATH,'//*[@id="Info_Tab_Contents"]/table/tbody/tr/td[9]/input').click()
        driver.find_element(By.ID,'disklock').click()
        driver.find_element(By.XPATH,'//*[@id="Doc_Menu"]/table/tbody/tr/td[2]/a/img').click()
        driver.find_element(By.XPATH,'//*[@id="BodyWrap_2"]/div/div[3]/form/table/tbody/tr/td[3]/a/span').click()
        driver.find_element(By.NAME,'Reason').send_keys("rpa_"+export_time.strftime("%Y-%m-%d_%H%M%S"))
        driver.find_element(By.XPATH,'//*[@id="BodyWrap_2"]/div/div[3]/div[1]/img[2]').click()
        time.sleep(5)
        alert = driver.switch_to.alert
        alert.accept()
        driver.close()
...
def web_export(path, browser):
    ...
    result = None
    try:
        #셀리니움driver.find_element_by_css_selector("input[type='file']").send_keys(path)
    ...
    for i in range(30):
        try:
            #셀리니움result = driver.find_element_by_xpath('//h2[contains(text(), "업로드 완료")]').text
            result = driver.find_element(By.XPATH, '//h2[contains(text(), "업로드 완료")]').text
