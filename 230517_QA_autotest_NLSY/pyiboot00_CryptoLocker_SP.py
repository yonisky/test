...
class Client_Test(unittest.TestCase):
    ...
    def test_040_pre_export_in_personal_drive(self):
        ...
        compare_export_drive = cmp(find_drive('개인문서함')+'pre_export.txt', find_drive('반출 보안디스크')+exp_folder_name+'\\pre_export.txt') #cmp -> Return True x permission denied , os.path 동일 실행
    ...
    def test_041_export_drive_when_cloudoc_login(self):
        export_drive = find_drive('반출 보안디스크')
    ...
    def test_044_secure_export_from_export_drive_to_web(self):
        exp_folder_name = recent_file_name(find_drive('개인문서함')+"DOC_EXPORT", 0)
        export_drive_file = find_drive('반출 보안디스크')+exp_folder_name+'\\pre_export.txt'
    ...
    def test_051_excel_file_create_in_online_secure_drive(self):      
        excel_create(find_drive('온라인 보안디스크')+now, 1)
        t1 = excel_read(find_drive('온라인 보안디스크')+now, 1)
        self.assertEqual(t1, "file write 1st")

    def test_052_excel_file_edit_in_online_secure_drive(self):
        excel_edit(find_drive('온라인 보안디스크')+now, 2)
        t1 = excel_read(find_drive('온라인 보안디스크')+now, 1)
        t2 = excel_read(find_drive('온라인 보안디스크')+now, 2)
        self.assertTrue(t1=="file write 1st" and t2=="file write 2nd")

    def test_053_winword_file_create_in_online_secure_drive(self):      
        winword_create(find_drive('온라인 보안디스크')+now, 'file write 1st')
        t1 = winword_read(find_drive('온라인 보안디스크')+now + '.docx')
        self.assertEqual(t1, "file write 1st\r\r")

    def test_054_winword_file_edit_in_online_secure_drive(self):      
        winword_edit(find_drive('온라인 보안디스크')+now + '.docx', 'file write 2nd')
        t1 = winword_read(find_drive('온라인 보안디스크')+now + '.docx')
        self.assertEqual(t1, "file write 1st\rfile write 2nd\r\r")

    def test_055_powerpnt_file_create_in_online_secure_drive(self):      
        powerpnt_create(find_drive('온라인 보안디스크')+now + '.pptx', 'file write 1st')
        t1 = powerpnt_read(find_drive('온라인 보안디스크')+now + '.pptx', 0)
        self.assertEqual(t1, "file write 1st")

    def test_056_powerpnt_file_edit_in_online_secure_drive(self):      
        powerpnt_edit(find_drive('온라인 보안디스크')+now + '.pptx', 'file write 2nd')
        t1 = powerpnt_read(find_drive('온라인 보안디스크')+now + '.pptx', 0)
        t2 = powerpnt_read(find_drive('온라인 보안디스크')+now + '.pptx', 1)
        self.assertTrue(t1=="file write 1st" and t2=="file write 2nd")

    def test_057_rename_file_in_online_secure_drive(self):
        f = open(find_drive('온라인 보안디스크')+'file_rename.txt', 'w')
        f.write('rename test')
        f.close()
        rename(find_drive('온라인 보안디스크'), 'file_rename')
        old_name = os.path.exists(find_drive('온라인 보안디스크')+'file_rename.txt')
        new_name = os.path.exists(find_drive('온라인 보안디스크')+'rename_file_rename.txt')
        f = open(find_drive('온라인 보안디스크')+'rename_file_rename.txt', 'r')
        data = f.read()
        f.close()
        self.assertTrue(old_name==0 and new_name==1 and data=='rename test')
        os.remove(find_drive('온라인 보안디스크')+'rename_file_rename.txt')

    def test_058_delete_file_in_online_secure_drive(self):
        f = open(find_drive('온라인 보안디스크')+'recyclebin_test.txt', 'w')
        f.write('delte and recycle test')
        f.close()
        delete_file(find_drive('온라인 보안디스크'), 'recyclebin_test.txt', 0)
        time.sleep(1)
        ...
        for value in recyclebin_list:
            if value.original_filename() == find_drive("온라인 보안디스크")+"recyclebin_test.txt":
                recyclebin_move = True
            elif value.original_filename() == find_drive("온라인 보안디스크")+"recyclebin_test":
                recyclebin_move = True
            else:
                pass
        self.assertTrue(os.path.exists(find_drive("온라인 보안디스크")+"recyclebin_test.txt")==0 and os.path.exists(find_drive("온라인 보안디스크")+"recyclebin_test")==0 and recyclebin_move==1)

    def test_059_restore_file_to_online_secure_drive(self):
        try:
            winshell.undelete(find_drive("온라인 보안디스크")+"recyclebin_test.txt")
        except:
            winshell.undelete(find_drive("온라인 보안디스크")+"recyclebin_test")
        recyclebin_restore = True
        drive_file_exist = os.path.exists(find_drive("온라인 보안디스크")+"recyclebin_test.txt")==1 or os.path.exists(find_drive("온라인 보안디스크")+"recyclebin_test")==1
        recyclebin_list = list(winshell.recycle_bin()) # returns list of objects of recycle-bin items 
        for value in recyclebin_list:
            if value.original_filename() == find_drive("온라인 보안디스크")+"recyclebin_test.txt":
        ...
        try:
            os.remove(find_drive("온라인 보안디스크")+"recyclebin_test.txt")
        except:
            os.remove(find_drive("온라인 보안디스크")+"recyclebin_test")
    ...
    def test_060_check_mount_offline_secure_drive(self):
        ...
        if not find_drive('오프라인 보안디스크') == None:
    ...
    def test_061_excel_file_create_in_offline_secure_drive(self):      
        excel_create(find_drive('오프라인 보안디스크')+"offline_"+now, 1)
        t1 = excel_read(find_drive('오프라인 보안디스크')+"offline_"+now, 1)
        self.assertEqual(t1, "file write 1st")

    def test_062_excel_file_edit_in_offline_secure_drive(self):
        excel_edit(find_drive('오프라인 보안디스크')+"offline_"+now, 2)
        t1 = excel_read(find_drive('오프라인 보안디스크')+"offline_"+now, 1)
        time.sleep(1)
        t2 = excel_read(find_drive('오프라인 보안디스크')+"offline_"+now, 2)
        self.assertTrue(t1=="file write 1st" and t2=="file write 2nd")

    def test_063_winword_file_create_in_offline_secure_drive(self):      
        winword_create(find_drive('오프라인 보안디스크')+"offline_"+now, 'file write 1st')
        t1 = winword_read(find_drive('오프라인 보안디스크')+"offline_"+now + '.docx')
        self.assertEqual(t1, "file write 1st\r\r")

    def test_064_winword_file_edit_in_offline_secure_drive(self):      
        winword_edit(find_drive('오프라인 보안디스크')+"offline_"+now + '.docx', 'file write 2nd')
        t1 = winword_read(find_drive('오프라인 보안디스크')+"offline_"+now + '.docx')
        self.assertEqual(t1, "file write 1st\rfile write 2nd\r\r")

    def test_065_powerpnt_file_create_in_offline_secure_drive(self):
        powerpnt_create(find_drive('오프라인 보안디스크')+"offline_"+now + '.pptx', 'file write 1st')
        time.sleep(1)
        t1 = powerpnt_read(find_drive('오프라인 보안디스크')+"offline_"+now + '.pptx', 0)
        time.sleep(1)
        self.assertEqual(t1, "file write 1st")

    def test_066_powerpnt_file_edit_in_offline_secure_drive(self):      
        powerpnt_edit(find_drive('오프라인 보안디스크')+"offline_"+now + '.pptx', 'file write 2nd')
        time.sleep(1)
        t1 = powerpnt_read(find_drive('오프라인 보안디스크')+"offline_"+now + '.pptx', 0)
        time.sleep(1)
        t2 = powerpnt_read(find_drive('오프라인 보안디스크')+"offline_"+now + '.pptx', 1)
        self.assertTrue(t1=="file write 1st" and t2=="file write 2nd")

    def test_067_rename_file_in_offline_secure_drive(self):
        f = open(find_drive('오프라인 보안디스크')+'file_rename.txt', 'w')
        f.write('rename test')
        f.close()
        rename(find_drive('오프라인 보안디스크'), 'file_rename')
        old_name = os.path.exists(find_drive('오프라인 보안디스크')+'file_rename.txt')
        new_name = os.path.exists(find_drive('오프라인 보안디스크')+'rename_file_rename.txt')
        f = open(find_drive('오프라인 보안디스크')+'rename_file_rename.txt', 'r')
        data = f.read()
        f.close()
        self.assertTrue(old_name==0 and new_name==1 and data=='rename test')
        os.remove(find_drive('오프라인 보안디스크')+'rename_file_rename.txt')
    ...
    def test_071_switch_to_external_network_mode(self):
        ...
        excel_create(find_drive('개인문서함')+'change networklock externel mode', 1)
        pyautogui.hotkey('ctrl', 'shift', 'alt', 'f10')
        time.sleep(1)                    #망 전환 후 시간(로딩)이 필요하다면 늘려야 하는 부분
        pyautogui.hotkey('alt','a')      #문서중앙화 접근 프로세스 종료여부 ON (설정홈)
    ...
    def test_075_excel_file_create_in_export_secure_drive(self):      
        excel_create(find_drive('반출 보안디스크')+now, 1)
        t1 = excel_read(find_drive('반출 보안디스크')+now, 1)
        self.assertEqual(t1, "file write 1st")

    def test_076_excel_file_edit_in_export_secure_drive(self):
        excel_edit(find_drive('반출 보안디스크')+now, 2)
        t1 = excel_read(find_drive('반출 보안디스크')+now, 1)
        t2 = excel_read(find_drive('반출 보안디스크')+now, 2)
        self.assertTrue(t1=="file write 1st" and t2=="file write 2nd")

    def test_077_winword_file_create_in_export_secure_drive(self):      
        winword_create(find_drive('반출 보안디스크')+now, 'file write 1st')
        t1 = winword_read(find_drive('반출 보안디스크')+now + '.docx')
        self.assertEqual(t1, "file write 1st\r\r")

    def test_078_winword_file_edit_in_export_secure_drive(self):      
        winword_edit(find_drive('반출 보안디스크')+now + '.docx', 'file write 2nd')
        t1 = winword_read(find_drive('반출 보안디스크')+now + '.docx')
        self.assertEqual(t1, "file write 1st\rfile write 2nd\r\r")

    def test_079_powerpnt_file_create_in_export_secure_drive(self):      
        powerpnt_create(find_drive('반출 보안디스크')+now + '.pptx', 'file write 1st')
        t1 = powerpnt_read(find_drive('반출 보안디스크')+now + '.pptx', 0)
        self.assertEqual(t1, "file write 1st")

    def test_080_powerpnt_file_edit_in_export_secure_drive(self):      
        powerpnt_edit(find_drive('반출 보안디스크')+now + '.pptx', 'file write 2nd')
        t1 = powerpnt_read(find_drive('반출 보안디스크')+now + '.pptx', 0)
        t2 = powerpnt_read(find_drive('반출 보안디스크')+now + '.pptx', 1)
        self.assertTrue(t1=="file write 1st" and t2=="file write 2nd")

    def test_081_rename_file_in_export_secure_drive(self):
        f = open(find_drive('반출 보안디스크')+'file_rename.txt', 'w')
        f.write('rename test')
        f.close()
        rename(find_drive('반출 보안디스크'), 'file_rename')
        old_name = os.path.exists(find_drive('반출 보안디스크')+'file_rename.txt')
        new_name = os.path.exists(find_drive('반출 보안디스크')+'rename_file_rename.txt')
        f = open(find_drive('반출 보안디스크')+'rename_file_rename.txt', 'r')
        data = f.read()
        f.close()
        self.assertTrue(old_name==0 and new_name==1 and data=='rename test')
        os.remove(find_drive('반출 보안디스크')+'rename_file_rename.txt')
    ...
    def test_082_internal_network_mount_drive(self):
        ...
        o_drive_check = find_drive('온라인 보안디스크') != None
        self.assertTrue(p_drive_check and t_drive_check and o_drive_check)
        pyautogui.hotkey('ctrl', 'shift', 'alt', 'f10')
        time.sleep(10)
        pyautogui.alert("네트워크락 테스트가 종료되었습니다.\nReportTest 에서 테스트 결과를 확인하세요.")

# %%
window=tkinter.Tk()
window.title("NETID_SQA_TEAM_SP")
